Presentatie Aantekeningen ABN AMRO AML
...
AML Cycle Placement : riskiest layer
...
Flows, each transaction goes in both flows Transactions -\> Rules based model -\> noise reduction ml model ---\> investigator -\> FUI Transactions -\> supervised models detect known forms of money laundering -\> unsupervised models that detect unknown anomalous behaviour -\> unsupervised models that detect anomalous behaviour ---\> investigator -\> FUI
...
Questions -1. can it make any model explainable? 0. can you explain what model you use to label data?

1. What models do you use within TM. (you mentioned combination of supervised and unsupervised)
2. Are you limited by Bias and Explainability to choose different approaches, if so what direction woudld you like to go in if explainability and or bias wouldn't be a requirement.
3. slides
4. evaluation metrics?
5. What decides what way takes a flow
6. when is precision mre important and when is recall more improtant?

Answers -1. Maybe 0. Rules based model for labelling data. ---\> but isnt using rules to label very erro prone -\> yes it is! thats why we want to change it to ml models

1. xgboost, decision trees, random trees, autoencoder, clustering, isolation forest
2. computational power and costs. why nn if XGBoost same performance. why use more complex models if xgboost are also really good
3. yes!
4. AUPR -\> Precision Recall Curve -\> good for imbalanced datasets
5. each transaction goese into both flows
6. Most cases focus on preciison, capture as many clients. but it is a difficult decision to make. All alerts are going to manual review so we have to discuess with them how many alerts are okay to review but we also want as much cases so we dicuss to find the best threshold.
